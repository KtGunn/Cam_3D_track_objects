...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int,int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
	...
	std::multimap<int,int> currBidKid; // multimap< <currentBoxID>,<currentKptID>>
	...
	for (auto itBM=prevBoxMatches.begin(); itBM!=prevBoxMatches.end(); itBM++) {
		...
		if (currBox.roi.contains (currKpt.pt)) {
		    // We have a match
		    currBidKid.insert (std::pair<int,int>(currBox.boxID,match.trainIdx));
		}
	    }
	}

	////////////////////////////////////////////////////////////////
	/// Select Current Bounding box with most matching keypoints
	//  to go with Previous Bounding box (top of loop)
	int maxCount = 0;
	int maxCountID = 0;

	
	if (currBidKid.size() > 0) {
	    for (auto i=currBidKid.begin(); i!=currBidKid.end(); i++) {
		// Is the count of present boxID higher than maxCount?
		if (currBidKid.count(i->first) > maxCount) {
		    // Yes, 
		    maxCount = currBidKid.count(i->first); // update max count
		    maxCountID = i->first; // associate ID w/max count
		}
	    }
	    bbBestMatches.insert (std::pair<int,int>(prevBBIndex,maxCountID));
	}
    }
    //
    return;
